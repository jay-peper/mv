...
foreach(@datei) {
  ...
  foreach (keys %Numbers) {
    if ($_ =~ /Vorname/ || $_ =~ /Nachname/ || $_ =~ /Straße_1/ ) {
    } else {
      say "$line[$Numbers{'Vorname'}] $line[$Numbers{'Nachname'}] $line[$Numbers{'Straße_1'}] - $_ = $line[$Numbers{$_}]";
      $sql = "UPDATE semkol SET \"".$_."\" = \"".$line[$Numbers{$_}]."\"";
      $sql .= " WHERE Vorname = \"".$line[$Numbers{'Vorname'}]. "\"";
      $sql .= " AND Nachname = \"" .$line[$Numbers{'Nachname'}]."\"";
      $sql .= " AND Straße_1 = \"" .$line[$Numbers{'Straße_1'}]."\"";
